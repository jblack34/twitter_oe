...
names <- page %>% html_nodes(".abgeordneter .name") %>% html_text(trim = TRUE)

# Extrahiere das Alter
ages <- page %>% html_nodes(".abgeordneter .name") %>% html_text(trim = TRUE)
ages <- sub(".*, (\\d+),.*", "\\1", ages) # Extrahiere nur die Altersangabe

# Extrahiere die Parteizugehörigkeit
parties <- page %>% html_nodes(".abgeordneter .partei") %>% html_text(trim = TRUE)
